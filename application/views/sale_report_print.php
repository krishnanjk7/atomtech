...
<html>
	...
	<?php  
		...
		$orderby = "customer_name"; //change this to whatever key you want from the array 
				...
				<div class="box box-primary">
					...
					<h4 style="text-align:center">
						<?php foreach($company as $row) {  ?>
							<?php if($_GET["cid"]==$row["id"]){  echo $row["name"]; } ?>
						<?php } ?>
					</h4>
					...
					<?php $old_customer=""; $total_amount=0; $total_received=0; $total_ounstanding=0; $old_customer_id=""; $old_invocie_date=""; ?>
							...
							<?php foreach($records as $row) { //var_dump($row); echo "\n"; ?>
								...
								<?php if($old_invocie_date!=""){ ?>
									...
									<?php foreach($sales_payment as $pay) {  ?>
										<?php if(($old_customer_id==$pay["customer_id"])&&(($pay["payment_date"]>=$old_invocie_date)&&($pay["payment_date"]<$row["invoice_date"]))){ ?>
											<tr>
												<td></td>
												<td></td>
												<td></td>
								...
								<?php }else{ ?>
									...
									<?php if($_GET["cid"]!="") { ?>
										<?php foreach($sales_payment as $pay) {  ?>
											<?php if(($row["customer_id"]==$pay["customer_id"])&&(($pay["payment_date"]<$row["invoice_date"]))){ ?>
												<tr>
													<td></td>
													<td></td>
													<td></td>
								...
								<?php if(($old_customer!="")&&($row["customer_name"]!=$old_customer)) { ?>
									
									<!--
									<?php foreach($sales_payment as $pay) {  ?>
										<?php if(($old_customer_id==$pay["customer_id"])&&($pay["payment_date"]>=$old_invocie_date)){ ?>
											<tr>
												<td>99</td>
												<td></td>
												<td></td>
								...
								<?php if(($old_customer=="")||($row["customer_name"]!=$old_customer)) { ?>
									<?php if(($_GET["cid"]=="")) { ?>
										<tr>
											<td class="bg-shade" colspan="3"><?php echo $row["customer_name"]; ?></td>
											<td class="bg-shade" colspan="2">Opening Balance</td>
											<?php if(isset($start_balance[$row["customer_id"]])) { ?>
											<td class="bg-shade text-right" ><?php echo rupeeformat($start_balance[$row["customer_id"]]); ?></td>
											<?php }else{ ?>
										...
										<?php foreach($sales_payment as $pay) {  ?>
											...
											<?php $total_ounstanding=$total_ounstanding-$pay["paid_amount"]; $total_received+=floatval($pay["paid_amount"]); } ?>
										<?php } ?>
									<?php $open_balance=isset($start_balance[$row["customer_id"]])?$start_balance[$row["customer_id"]]:0;  ?>
									<?php } ?>
								<?php } ?>
								
								<?php	
									$old_customer_id=$row["customer_id"]; 
									$old_customer=$row["customer_name"]; 
